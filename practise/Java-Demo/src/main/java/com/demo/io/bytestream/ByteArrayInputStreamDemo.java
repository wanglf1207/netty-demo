...
public class ByteArrayInputStreamDemo {
    ...
    /**
     * 定义字节数组,7A对应十进制是122，小于byte最大值127
    ...
    public static void byteArrayInputStream() {
        ...
        for (int i=0; i<5; i++) {
            ...
            // available() 返回剩余可读取字节个数
            if (bais.available() >= 0) {
                // 读取“字节流的下一个字节” 当读到字母a的时候，tmp的值是ASCII码值是97，
                // 然后通过Integer.toHexString方法转化成十六进制的61
                int tmp = bais.read();
                System.out.printf("%d : 0x%s\n", tmp, Integer.toHexString(tmp));
            }
        }

        System.out.printf("读取五个字符之后长度变为:%d\n",bytes.length);

        /**
         * 测试InputStream是否支持标记/复位。
         * 该markSupported的方法ByteArrayInputStream总是返回true 。
         */
        System.out.println("markSupported " + bais.markSupported());

        /**
         * 标记“字节流中下一个被读取的位置”。即--标记“0x66”，因为前面已经读取了5个字节，
         * 所以下一个被读取的位置是第6个字节”
         * (01), ByteArrayInputStream类的mark(0)函数中的“参数0”是没有实际意义的。
         * (02), mark()与reset()是配套的，reset()会将“字节流中下一个被读取的位置
         * ”重置为“mark()中所保存的位置”
         */
        bais.mark(0);
